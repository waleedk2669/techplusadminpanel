...
const DispatchedOrders = () => {
  ...
  useEffect(() => {
    dispatch(fetchNewOrdersRequest({ id: 4, rowsPerPage: rowsPerPage }));
    setLoading(true);
  ...
  const onDeliver = (e) => {
    dispatch(changeStatusRequest({tab_id: 4, medicine_ids: selectedRowIds}))
    console.log({status: 1, tab_id: 4, medicine_ids: selectedRowIds})
    addSnackbar('Medicines Delivered Successfully', 'success');
    dispatch(fetchNewOrdersRequest({ id: 4, rowsPerPage: rowsPerPage }));
  }
  return (
    <Grid container spacing={0}>
      {
        ...
        <>
          <Grid item xs={12}>
            <DataTable
            extraButtonText={"Deliver"}
            extraButtonHandler={onDeliver}
